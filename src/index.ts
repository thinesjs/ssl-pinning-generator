...
async function decryptSslPinning(
    ...
    try {
        ...
        const decrypted = await jose.JWE.createDecrypt(key).decrypt(
            encryptedData
        );
...
async function readSslPinningFromUrl(
    ...
    try {
        ...
        // Decrypt the data
        const decryptedConfig = await decryptSslPinning(
            encryptedData.trim(),
            privateKeyPath
        );
        ...
        console.error(
            "Error occurred while reading SSL pinning from URL:",
            error
        );
        throw error;
    }
...
async function exportSslPinningFromUrl(
    ...
    try {
        ...
        console.log(
            `Successfully exported decrypted SSL pinning configuration to: ${outputPath}`
        );
        ...
        console.error(
            "Error occurred while exporting SSL pinning from URL:",
            error
        );
        throw error;
    }
...
async function main() {
    ...
    if (operation === "read") {
        ...
        const url = "https://assets.example.com/certs/ssl-pinning";
        ...
        try {
            try {
                ...
                console.error(
                    `Private key file not found: ${privateKeyPath}. Please ensure this file exists for decryption.`
                );
                process.exit(1);
            }
            ...
            console.error(
                "Failed to read and decrypt SSL pinning configuration:",
                error
            );
            process.exit(1);
        }
        ...
        try {
            try {
                ...
                console.error(
                    "Input files not found. Please check the file paths."
                );
                process.exit(1);
            }
            ...
            console.error(
                "Failed to encrypt SSL pinning configuration:",
                error
            );
            process.exit(1);
        }
